...
- (void)update {    
    ...
    moveScore = [self.moveEngine summary];
    sleepScore = [self.sleepEngine summary];
    ...
    self.scoreHGraphView.hGraphEatScore = eatScore;
    self.scoreHGraphView.hGraphMoveScore = moveScore;
    self.scoreHGraphView.hGraphSleepScore = sleepScore;
    ...
    self.sleepScoreBarView.barViewSleepScore = sleepScore;
    if (curGraphKind == MD_BAR_SLEEP) {
        self.bvScore.text = [NSString stringWithFormat:@"%d%%", (int)sleepScore];
        self.bvGoal.text = [NSString stringWithFormat:@"TODAY'S GOAL: 7 HOURS"];
        self.bvTitle.text = [NSString stringWithFormat:@"SLEEP"];
        NSLog(@"Update sleep bar view, bvScore %@", self.bvScore.text );
    }
...
- (void)hideAllGraphs {
    ...
    // sleep
    self.sleepScoreBarView.hidden = YES;
...
- (void)unhideBarSleep {
    self.sleepScoreBarView.hidden = NO;
    self.bvTitle.hidden = NO;
    self.bvScore.hidden = NO;
    self.bvGoal.hidden = NO;
}
